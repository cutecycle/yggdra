...
// ── Panel system modules ──────────────────────────────────────────
pub mod panels;
pub mod render_panels;
pub mod stream;

pub use panels::PanelLayout;
...
pub struct App {
    ...
    /// Consecutive empty tool outputs (whitespace-only or zero-length) — blocked detection
    consecutive_empty_tool_outputs: u32,
    ...
    // ── Ephemeral panel system ────────────────────────────────────────
    /// Multi-column ephemeral panel layout
...
impl App {
    ...
    pub fn new(
        ...
        // Initialize panel layout with current terminal size
        let (term_width, term_height) = crossterm::terminal::size()
            .unwrap_or((120, 24));
        let mut panel_layout = PanelLayout::new(term_width, term_height);
        panel_layout.recalculate(term_width, term_height);
    ...
    /// Update or create a panel with the given content.
    /// 
    /// # Arguments
    /// * `name` - Panel identifier (alphanumeric + underscores)
    /// * `content` - Content to display in the panel
    /// * `clear` - Whether to clear existing content before adding (if false, content replaces)
    /// * `position` - Optional position hint ("left" or "right"). If None, auto-detects based on terminal width
    /// 
    /// # Returns
    /// Success message if panel was updated; error if validation fails
    pub fn update_panel(
        &mut self,
        column: u16,
        name: String,
        ...
        if name.is_empty() {
            return Err(anyhow::anyhow!("panel: panel name cannot be empty"));
        }
        ...
        // Validate column
        let num_cols = self.panel_layout.metrics.num_columns;
        let center_col = self.panel_layout.metrics.center_column;
        ...
        if column == center_col {
            return Err(anyhow::anyhow!("Cannot place panel in center column (reserved for main chat)"));
        }

        // Update panel content
        if clear {
            // Replace content entirely by creating a new panel
            let mut panel = crate::ui::panels::EphemeralPanel::new(name.clone(), crate::ui::panels::PanelPosition::Left);
            panel.content = content.clone();
            self.panel_layout.create_or_update_panel(column, name.clone(), panel)
                .map_err(|e| anyhow::anyhow!("{}", e))?;
        } else {
            // Append to existing content or create new
            if let Some(panel) = self.panel_layout.get_panel_mut(column, &name) {
                panel.content.push_str(&content);
            } else {
                let mut panel = crate::ui::panels::EphemeralPanel::new(name.clone(), crate::ui::panels::PanelPosition::Left);
                panel.content = content.clone();
                self.panel_layout.create_or_update_panel(column, name.clone(), panel)
        ...
        // Mark layout as dirty so it gets re-rendered
        self.panels_dirty = true;

        Ok(format!(
            "✅ Panel '{}' updated: column={}, bytes={}",
            name,
            column,
            self.panel_layout.get_panel(column, &name)
                .map(|p| p.content.len())
                .unwrap_or(0)
        ))
    }
        ...
        for (msg_idx, msg) in self.messages_cache.iter().enumerate() {
            ...
            let content = if is_tool_msg {
                ...
                let diff_content = if let Some(rest) = msg.content.strip_prefix("[TOOL_OUTPUT: ") {
                    if let Some(eq) = rest.find(" = ") {
                        ...
                        if looks_like_diff(raw_body) {
                            ...
                            let highlighted = self.render_tool_output_highlighted(
                                name,
                                raw_body,
                                is_expanded,
                                is_cursor,
                ...
                if let Some(t) = diff_content {
                    ...
                    // Fallback for non-[TOOL_OUTPUT:...] tool messages (e.g., [TOOL_ERROR:...])
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        if self.mode == AppMode::Forever {
            ...
            if should_kick {
                ...
                crate::dlog!("turn_phase: Idle → Streaming | msg_count={} | mode={:?}", messages.len(), self.mode);
                let (_fits, usage_pct) = self.check_context_before_stream(&messages, Some(&steering));
                crate::dlog!("context_check: fits={} usage_pct={}%", _fits, usage_pct);
                ...
                if let Some(client) = &self.ollama_client {
                    ...
                    crate::dlog!("streaming_start: calling generate_streaming | tool_cap={:?} ctx_win={:?}", tool_cap, ctx_win);
        ...
        loop {
            ...
            // Poll streamed panels every 10 ticks (~100ms at 10ms/frame) for performance
            if self.tick_count % 10 == 0 {
                self.check_streamed_panels();
            }
            ...
            'events: loop {
                if crossterm::event::poll(Duration::ZERO)? {
                    match event::read()? {
                        ...
                        Event::Resize(w, h) => {
                            // Recalculate panel layout on terminal resize
                            self.panel_layout.recalculate(w, h);
                            self.panels_dirty = true;
        ...
        // Clean up all active streamed processes before exit
    ...
    fn complete_streaming_turn(&mut self) {
        ...
        if self.streaming_text.is_empty() {
            ...
            // If we have thinking text but no streaming text, just kick without counting
            // toward completion. Thinking-only responses should not auto-complete one mode.
            if !self.thinking_text.is_empty() {
                self.push_agent_notice("💭 Processing thinking — continuing.".to_string());
                self.inject_continue_kick();
                return;
            }
            ...
            match action {
                StreamEndAction::CompleteOne(reason) => {
                    self.push_agent_notice(
                        "⚠️ Empty responses — completing task.".to_string()
                    );
    ...
    fn check_tool_result(&mut self) {
        ...
        match rx.try_recv() {
            Ok(result) => {
                ...
                // Handle panel tool specially: parse parameters and update panel state
                if result.tool_name == "panel" && result.output.is_ok() {
                    let parts: Vec<&str> = result.args.splitn(6, '\x00').collect();
                    if parts.len() >= 1 {
                        ...
                        let clear_str = parts.get(5).map(|s| s.to_lowercase()).unwrap_or_default();
                        
                        let stream = matches!(stream_str, "true" | "1" | "yes");
                        let clear = matches!(clear_str.as_str(), "true" | "1" | "yes");
                        
                        // Parse column as u16
                        match column_str.parse::<u16>() {
                            Ok(column) => {
                                if stream && !command.is_empty() {
                                    // Streaming mode: spawn child process and create streamed panel
                                        ...
                                        .spawn()
                                    {
                                        Ok(child) => {
                                            ...
                                            let child_future = crate::ui::stream::spawn_panel_stream_task(child, tx);
                                            let panel_name_clone = panel_name.clone();
                                            
                                            // Create streamed panel
                                            if let Ok(panel) = self.panel_layout.create_streamed_panel(panel_name.clone(), column) {
                                                panel.output_receiver = Some(rx);
                                                panel.title = Some(format!("▶ {}", command));
                                                // Spawn the streaming task in background
                                                tokio::spawn(async move {
                                                    match child_future.await {
                                                        Ok(exit_code) => {
                                                            crate::dlog!("✅ Panel stream '{}' finished with exit code {}", panel_name_clone, exit_code);
                                                        }
                                                        Err(e) => {
                                                            crate::dlog!("❌ Panel stream '{}' error: {}", panel_name_clone, e);
                                                        }
                                                    }
                                                });
                                                crate::dlog!("📡 Streaming panel '{}' started: {}", panel_name, command);
                                            } else {
                                                crate::dlog!("⚠️  panel tool: failed to create streamed panel '{}'", panel_name);
                                            }
                                        }
                                        Err(e) => {
                                            crate::dlog!("⚠️  panel tool: failed to spawn command '{}': {}", command, e);
                                        }
                                    }
                                } else {
                                    // Static mode: call update_panel
                                    if let Err(e) = self.update_panel(column, panel_name, content, clear) {
                                        crate::dlog!("⚠️  panel tool: {}", e);
                                    }
                                }
                            }
                            Err(_) => {
                                crate::dlog!("⚠️  panel tool: invalid column '{}' (expected integer)", column_str);
                            }
                        }
                    }
                }
                ...
                if self.mode == AppMode::Ask {
                    ...
                    let is_readonly = matches!(result.tool_name.as_str(), "rg" | "exec" | "shell" | "readfile" | "panel");
                ...
                // Track empty tool outputs for blocked detection
                match &result.output {
                    Ok(output) => self.track_tool_output_emptiness(output),
                    Err(_) => self.track_tool_output_emptiness(""),  // Treat errors as empty
                }
    ...
    fn memory_block() -> String {
        ...
        crate::dlog!("memory_block: reading from {:?}", path);
        let content = match std::fs::read_to_string(&path) {
            Ok(c) => {
                crate::dlog!("memory_block: read {} bytes", c.len());
                c
            }
            Err(_) => {
                crate::dlog!("memory_block: file not found or unreadable");
                return String::new();
            }
        };
        if content.trim().is_empty() {
            crate::dlog!("memory_block: empty content");
            return String::new();
        }
    ...
    /// Contents of .yggdra/session_notes.md (generated by /compress).
    /// Persists session summary across restarts to maintain context.
    fn session_notes_block() -> String {
        let root = crate::sandbox::project_root()
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| std::path::PathBuf::from("."));
        let path = root.join(".yggdra/session_notes.md");
        crate::dlog!("session_notes_block: reading from {:?}", path);
        let content = match std::fs::read_to_string(&path) {
            Ok(c) => {
                crate::dlog!("session_notes_block: read {} bytes", c.len());
                c
            }
            Err(_) => {
                crate::dlog!("session_notes_block: file not found or unreadable");
                return String::new();
            }
        };
        if content.trim().is_empty() {
            crate::dlog!("session_notes_block: empty content");
            return String::new();
        }
        format!("SESSION NOTES (.yggdra/session_notes.md — auto-generated by /compress):\n{}\n", content)
    }
    ...
    fn thoughts_block() -> String {
        ...
        crate::dlog!("thoughts_block: reading from {:?}", path);
        let content = match std::fs::read_to_string(&path) {
            Ok(c) => {
                crate::dlog!("thoughts_block: read {} bytes", c.len());
                c
            }
            Err(_) => {
                crate::dlog!("thoughts_block: file not found or unreadable");
                return String::new();
            }
        };
        if content.trim().is_empty() {
            crate::dlog!("thoughts_block: empty content");
            return String::new();
        }
        let lines: Vec<&str> = content.lines().collect();
    ...
    /// Format visible panels for injection into the system prompt.
    /// Lists which panels are currently on-screen to help agents make informed decisions about panel placement.
    fn format_visible_panels(&self) -> String {
        let mut output = String::from("\n=== VISIBLE PANELS ===\n");
        let mut has_panels = false;

        crate::dlog!("format_visible_panels: columns.len() = {}, streamed.len() = {}", 
            self.panel_layout.columns.len(), self.panel_layout.streamed_panels.len());

        // List static panels in each column
        for (&col_idx, panels) in &self.panel_layout.columns {
            for (_name, panel) in panels {
                crate::dlog!("format_visible_panels: static panel '{}' visible={}", _name, panel.visible);
                if panel.visible {
                    has_panels = true;
                    let type_str = "static";
                    let preview = panel.content
                        .lines()
                        .next()
                        .unwrap_or("(empty)")
                        .chars()
                    ...
                    output.push_str(&format!(
                        "- Column {}: {} ({}) - {}\n",
                        col_idx, panel.name, type_str, preview
                    ));
                }
            }
        }

        // List streamed panels
        for (_name, stream_panel) in &self.panel_layout.streamed_panels {
            crate::dlog!("format_visible_panels: streamed panel '{}' is_streaming={}", _name, stream_panel.is_streaming);
            has_panels = true;
            let status = if stream_panel.is_streaming {
                "[streaming...]"
            } else if let Some(code) = stream_panel.exit_code {
                &format!("[done: code {}]", code)
            } else {
                "[done]"
            };
            let line_count = stream_panel.accumulated_output.lines().count();
            let size_kb = stream_panel.accumulated_output.len() / 1024;
            output.push_str(&format!(
                "- Column {}: {} {} - {} lines, {}KB\n",
                stream_panel.column, stream_panel.panel_name, status, line_count, size_kb
            ));
        }

        if !has_panels {
            output.push_str("(No visible panels)\n");
        }

        crate::dlog!("format_visible_panels: has_panels={}", has_panels);
        output
    }
    ...
    fn steering_text(&mut self) -> String {
        ...
        let mode_block = match self.mode {
            ...
            AppMode::Plan =>
                "MODE: PLAN (interactive). Discuss and analyse. Read-only commands in shell only. \
                 Every response: start with clarity level (1-100: 1=confused, 100=fully clear). \
                 When the task is clear, emit </understood> to proceed.",
            AppMode::Forever =>
                "MODE: BUILD (autonomous). Execute shell commands to complete tasks. \
                 Signal completion with </done>. \
                 If output is sparse, explore via AGENTS.md context, file structure variations, or error context below.",
            AppMode::One =>
        ...
        let _ctx_note = self.config.params.num_ctx
            .or(self.config.context_window)
            .map(|n| format!("\nCONTEXT WINDOW: {} tokens — budget returnlines accordingly", n))
            .unwrap_or_default();
        let mut base = format!(
            "yggdra — the adorable TUI agent 🌸✨\n\
             You are yggdra, the most adorable agent in the universe! Be warm, friendly, and encouraging. \
             Keep responses concise and delightful.\n\
             {mode_block}\n\
             ---\n"
        );
        ...
        // Async instructions
        base.push_str(
            "⚡ ASYNC MODE — Run commands in background:\n\
             Add <mode>async</mode> and <task_id>my-task</task_id> to any shell call.\n\
             Get immediate ack, continue working. Result: [ASYNC_RESULT: my-task = ...] when done.\n\
             Use for: downloads, long builds, tests, packages, large files.\n\
             ---\n"
        );
        ...
        base.push_str(
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\
             CODE EDITING — Patterns for reliable modifications:\n\
             ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\
             1. Read first — get exact lines:\n\
                cat -n src/foo.rs | sed -n '25,40p'\n\
             2. Single-line replace:\n\
             ...
             Verify: cat -n f.rs | sed -n 'N,Mp'\n\
             → sed regex on Rust is error-prone; use awk line-numbers or python3.replace\n\
             ---\n"
        );
        ...
        if !session_notes.is_empty() {
            ...
            crate::dlog!("steering_text: session_notes_block added ({} bytes)", session_notes.len());
        }
        crate::dlog!("steering_text: checking agents_context");
        ...
        base.push_str("\n→ File tree is live — go directly to relevant files.");
        ...
        // Inject visible panels so agent knows what's on screen
        let panels_block = self.format_visible_panels();
        base.push_str(&panels_block);
        ...
        base.push_str("---\n\
             ...
             COMPLETION & PROGRESS\n\
             ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\
             • </done> — Signal task completion (BUILD/ONE modes)\n\
             • </understood> — Signal ready to proceed (PLAN mode)\n\
             • <percent>N</percent> — Show progress 0-100 in UI loading bar\n\
             • Output XML tool tags only, zero prose outside tags");
        crate::dlog!("steering_text: COMPLETE | total_bytes={}", base.len());
    ...
    /// Check if we've received 3 consecutive empty tool outputs (whitespace-only or zero-length).
    /// If so, inject a steering directive, send notification, and reset counter.
    /// Only applies in Build (Forever) and One modes.
    fn check_and_handle_empty_responses(&mut self) {
        // Only in autonomous modes
        if !matches!(self.mode, AppMode::Forever | AppMode::One) {
            return;
        }

        // Already handled in this cycle
        if self.consecutive_empty_tool_outputs < 3 {
            return;
        }

        // Fire the notification asynchronously
        let notification_msg = "Agent received 3 empty responses in a row. \
                               Check if blocked: provide guidance or use /abort to retry";
        let msg_clone = notification_msg.to_string();
        tokio::spawn(async move {
            crate::notifications::agent_blocked(&msg_clone).await;
        });

        // Add a notice to the chat
        self.push_agent_notice(
            "⚠️ Three empty tool responses — likely blocked or confused. \
             Please provide guidance, clarify the task, or use /abort to recover."
                .to_string(),
        );

        // Inject steering directive as a system message
        let steering_msg = Message::new(
            "user",
            "You have received 3 empty responses in a row from tools. \
             This likely means you are blocked or need to take a completely different approach. \
             Please communicate your current status, what you tried, and what blockers you encountered. \
             If you are stuck, ask the human for help or clarification.",
        );

        if let Err(e) = self.message_buffer.add_and_persist(steering_msg) {
            crate::dlog!("Failed to persist steering message: {}", e);
        } else {
            self.cached_message_count = self.message_buffer.count()
                .unwrap_or(self.cached_message_count + 1);
        }

        // Reset the counter so we don't spam
        self.consecutive_empty_tool_outputs = 0;
    }

    /// Called after a tool result is processed to track empty outputs
    fn track_tool_output_emptiness(&mut self, output: &str) {
        let is_empty = output.trim().is_empty();
        
        if is_empty {
            self.consecutive_empty_tool_outputs += 1;
        } else {
            self.consecutive_empty_tool_outputs = 0;
        }

        // Trigger the handler if we've hit 3 consecutive empty responses
        if self.consecutive_empty_tool_outputs == 3 {
            self.check_and_handle_empty_responses();
        }
    }
    ...
    /// Calculate N-column areas for rendering (one Rect per column).
    fn calculate_n_column_areas(&self, total_area: Rect) -> Vec<Rect> {
        let num_columns = self.panel_layout.metrics.num_columns as usize;
        
        // Build constraints: each column gets 96 chars, except the last column
        // which gets Min() to absorb remaining terminal width.
        let mut constraints = Vec::with_capacity(num_columns);
        for i in 0..num_columns {
            if i == num_columns - 1 {
                constraints.push(Constraint::Min(96));
            } else {
                constraints.push(Constraint::Length(96));
            }
        }
    ...
    fn draw(&self, f: &mut Frame) {
        ...
        // ── Apply horizontal N-column layout ──
        let column_areas = self.calculate_n_column_areas(area);
        let num_columns = self.panel_layout.metrics.num_columns as usize;
        let center_col = self.panel_layout.metrics.center_column as usize;
        
        // The main_area is the center column where all the main chat UI goes
        let main_area = column_areas[center_col];
        ...
        let _exchange_idx = self.render_cache_exchange_end;
        ...
        // ── Render ephemeral side panels for all non-center columns ──
        for col_idx in 0..num_columns {
            if col_idx != center_col {
                render_panels::render_side_panel(f, column_areas[col_idx], &self.panel_layout, col_idx as u16);
            }
        }
    }
    ...
    async fn handle_command(&mut self) {
        ...
        if command.starts_with("/tool ") {
            ...
            self.consecutive_empty_tool_outputs = 0; // Reset empty response tracking
            self.autokick_paused = false;
            self.consecutive_format_errors = 0;
    ...
    async fn handle_compress(&mut self) {
        ...
        // Written to .yggdra/session_notes.md — automatically injected into the system
        // prompt via session_notes_block() on every turn. This maintains context across
        // compress operations and even after the session is restarted.
    ...
    /// Poll all active streamed panels for new output.
    ///
    /// For each streamed panel with an active output receiver:
    /// - Drain all pending messages (non-blocking try_recv loop)
    /// - Append each line to the panel's accumulated_output
    /// - Mark panels_dirty when output is received (triggers re-render)
    /// - Handle channel disconnect (process finished)
    /// - Clean up receiver after disconnect
    fn check_streamed_panels(&mut self) {
        if self.panel_layout.streamed_panels.is_empty() {
            return;
        }

        let panel_names: Vec<String> = self.panel_layout.streamed_panels.keys().cloned().collect();
        let mut had_output = false;

        for panel_name in panel_names {
            if let Some(panel) = self.panel_layout.streamed_panels.get_mut(&panel_name) {
                let mut lines_to_append = Vec::new();
                let mut should_mark_done = false;

                {
                    if let Some(rx) = &mut panel.output_receiver {
                        loop {
                            match rx.try_recv() {
                                Ok(line) => {
                ...
                // Apply accumulated lines and cleanup state
    ...
    /// Kill a single streamed panel process gracefully with timeout fallback.
    ///
    /// Attempts to send SIGTERM to the process. If it doesn't terminate within 2 seconds,
    /// sends SIGKILL to force termination. Logs any errors encountered.
    async fn kill_streamed_panel(&mut self, panel_name: &str) {
        if let Some(panel) = self.panel_layout.remove_streamed_panel(panel_name) {
            if let Some(mut child) = panel.process {
                // Try graceful SIGTERM first
                match child.kill().await {
                    Ok(_) => {
                        crate::dlog!("Sent SIGTERM to streamed panel '{}'", panel_name);
                        ...
                        match timeout {
                            Ok(Ok(status)) => {
                                crate::dlog!(
                                    "Streamed panel '{}' terminated with status: {}",
                                    panel_name,
                                    status.code().unwrap_or(-1)
                                );
                            }
                            Ok(Err(e)) => {
                                crate::dlog!("Error waiting for streamed panel '{}': {}", panel_name, e);
                            }
                            Err(_) => {
                                // Timeout: process did not terminate after SIGTERM
                                // SIGKILL is already in flight from kill().await, but let's log it
                                crate::dlog!(
                                    "Streamed panel '{}' did not terminate within 2s after SIGTERM (SIGKILL in effect)",
                                    panel_name
                                );
                            }
                        }
                    ...
                    Err(e) => {
                        crate::dlog!("Failed to kill process for streamed panel '{}': {}", panel_name, e);
                    }
                }
            }
        }
    ...
    /// Kill all active streamed panel processes gracefully on app shutdown.
    ///
    /// Iterates through all streamed panels and kills each process gracefully.
    /// Handles async cleanup without blocking the main loop.
    async fn cleanup_all_streams(&mut self) {
        if self.panel_layout.streamed_panels.is_empty() {
            return;
        }

        crate::dlog!(
            "Cleaning up {} streamed panel(s)",
            self.panel_layout.streamed_panels.len()
        );

        // Collect panel names to avoid borrowing issues during iteration
        let panel_names: Vec<String> = self.panel_layout.streamed_panels.keys().cloned().collect();

        for panel_name in panel_names {
            self.kill_streamed_panel(&panel_name).await;
        }

        crate::dlog!("All streamed panels cleaned up");
    }
    ...
    /// Render tool output with syntax highlighting based on tool name
    fn render_tool_output_highlighted(
        &self,
        tool_name: &str,
        output: &str,
        expanded: bool,
        ...
        // Detect language from tool name and use highlighter as fallback
        let lang = match tool_name.to_lowercase().as_str() {
            "shell" | "exec" => "bash",
            "python" => "python",
            "rg" | "grep" => {
                // For search tools, detect from output content
                crate::highlight::detect_language(output)
            }
            "editfile" | "setfile" => {
                // For file operations, detect from content
                crate::highlight::detect_language(output)
            }
            _ => crate::highlight::detect_language(output),
        };

        // Detect and strip trailing truncation marker
        let (body, truncation_note) = if let Some(trunc_pos) = output.rfind("...(truncated to ") {
            let note = output[trunc_pos + 3..].trim_end_matches(')');
            let clean = output[..trunc_pos].trim_end_matches('.');
            (clean, Some(format!("✂️  {}", note)))
        } else {
            (output, None)
        };

        // Render tool header: "🔧 tool_name (N lines, M chars)"
        let output_lines: Vec<&str> = body.lines().collect();
        let total_lines = output_lines.len();
        let total_chars = body.len();

        let header_style = Style::default().fg(if is_dark {
        ...
        lines.push(Line::from(Span::styled(
            format!("🔧 {}  ({} lines, {} chars)", tool_name, total_lines, total_chars),
            header_style,
        )));

        // Highlight the output
        let highlighted_lines = if expanded {
            self.highlighter.highlight_code(body, lang, is_dark)
        } else {
            // For collapsed view, only highlight first 3 lines
            let preview = output_lines
                ...
                .collect::<Vec<_>>()
                .join("\n");
            self.highlighter.highlight_code(&preview, lang, is_dark)
        };

        lines.extend(highlighted_lines);

        // Add "more lines" indicator if collapsed and there are more lines
        if !expanded && total_lines > 3 {
            let more_style = Style::default().fg(if is_dark {
                Color::Rgb(92, 99, 112)
            } else {
                Color::Rgb(140, 148, 158)
            });
            lines.push(Line::from(Span::styled(
                format!("│   … ({} more lines, {} chars)", total_lines - 3, total_chars),
                more_style,
            )));
        }

        // Add truncation note if present
        if let Some(note) = truncation_note {
            let trunc_style = Style::default().fg(if is_dark {
        ...
        // Add expand/collapse hint if cursor is on this message
        if is_cursor {
            let hint_style = Style::default().fg(if is_dark {
                Color::Rgb(92, 99, 112)
            } else {
                Color::Rgb(140, 148, 158)
            });
            let hint = if expanded { "  [Space=collapse]" } else { "  [Space=expand]" };
...
#[cfg(test)]
mod visible_panels_tests {
    use super::*;
    use crate::ui::panels::{EphemeralPanel, PanelPosition};

    #[test]
    fn test_format_visible_panels_no_panels() {
        // Create a minimal app-like object for testing
        let mut panel_layout = crate::ui::panels::PanelLayout::new(80, 24);
        
        // Since we can't test the full method without App, we'll test by manually building the output
        let mut output = String::from("\n=== VISIBLE PANELS ===\n");
        
        // List static panels in each column
        for (&col_idx, panels) in &panel_layout.columns {
            for (_name, panel) in panels {
                if panel.visible {
                    let type_str = "static";
                    let preview = panel.content
        ...
        // List streamed panels
        for (_name, stream_panel) in &panel_layout.streamed_panels {
            let status = if stream_panel.is_streaming {
                "[streaming...]"
            } else if let Some(code) = stream_panel.exit_code {
                &format!("[done: code {}]", code)
        ...
        if !panel_layout.columns.is_empty() || !panel_layout.streamed_panels.is_empty() {
            // has panels
        } else {
            output.push_str("(No visible panels)\n");
        }
        
        assert!(output.contains("=== VISIBLE PANELS ==="));
        assert!(output.contains("(No visible panels)"));
    }

    #[test]
    fn test_format_visible_panels_with_static_panel() {
        // Use larger terminal width to get multiple columns (96*2 = 192)
        let mut panel_layout = crate::ui::panels::PanelLayout::new(300, 24);
        
        // Create a static panel - put it in column 0 (left side)
        ...
        let _ = panel_layout.create_or_update_panel(0, "test-panel".to_string(), panel);
        ...
        assert!(output.contains("=== VISIBLE PANELS ==="));
        assert!(output.contains("Column 0:"));
    ...
    #[test]
    fn test_format_visible_panels_with_hidden_panel() {
        let mut panel_layout = crate::ui::panels::PanelLayout::new(300, 24);
        
        // Create a hidden panel
        let panel = EphemeralPanel {
            name: "hidden-panel".to_string(),
            content: "This content should not appear".to_string(),
            visible: false,
            position: PanelPosition::Left,
            priority: 100,
            updated_at: std::time::Instant::now(),
        ...
        let _ = panel_layout.create_or_update_panel(0, "hidden-panel".to_string(), panel);
        ...
        assert!(output.contains("(No visible panels)"));
        assert!(!output.contains("hidden-panel"));
    }

    #[test]
    fn test_format_visible_panels_preview_truncation() {
        let mut panel_layout = crate::ui::panels::PanelLayout::new(300, 24);
        
        // Create a panel with very long first line
        let long_content = "a".repeat(100);
        let panel = EphemeralPanel {
            name: "long-panel".to_string(),
            content: format!("{}\nSecond line", long_content),
            visible: true,
            position: PanelPosition::Left,
            priority: 100,
            updated_at: std::time::Instant::now(),
        ...
        let _ = panel_layout.create_or_update_panel(0, "long-panel".to_string(), panel);
        ...
        assert!(output.contains("Column 0:"));
        assert!(output.contains("long-panel"));
        // Check that the preview is limited (max 50 chars of content)
        let preview_part = output.split("long-panel").nth(1).unwrap_or("");
        let preview_line = preview_part.lines().next().unwrap_or("");
        // The preview line should contain the panel name and limited content
        assert!(preview_line.len() < 150); // Should be much shorter than the full content
    }

    #[test]
    fn test_format_visible_panels_multiple_columns() {
        // Use larger terminal width to get 3+ columns
        let mut panel_layout = crate::ui::panels::PanelLayout::new(400, 24);
        
        // Create panels in different columns
        let panel1 = EphemeralPanel {
            name: "left-panel".to_string(),
        ...
        let panel2 = EphemeralPanel {
            name: "right-panel".to_string(),
            content: "Right content".to_string(),
            visible: true,
        ...
        let _ = panel_layout.create_or_update_panel(0, "left-panel".to_string(), panel1);
        // Put right panel at the rightmost column (should be 3 or 4)
        let rightmost_col = panel_layout.metrics.num_columns - 1;
        let _ = panel_layout.create_or_update_panel(rightmost_col, "right-panel".to_string(), panel2);
        ...
        assert!(output.contains("Column 0:"));
        assert!(output.contains("left-panel"));
        assert!(output.contains("right-panel"));
    }

    #[test]
    fn test_format_visible_panels_empty_content() {
        let mut panel_layout = crate::ui::panels::PanelLayout::new(300, 24);
        
        // Create a panel with empty content
        let panel = EphemeralPanel {
            name: "empty-panel".to_string(),
            content: String::new(),
            visible: true,
            position: PanelPosition::Left,
            priority: 100,
        ...
        let _ = panel_layout.create_or_update_panel(0, "empty-panel".to_string(), panel);
        ...
        assert!(output.contains("Column 0:"));
        assert!(output.contains("empty-panel"));
...
// ============================================================================
// End-to-end regression tests for one-mode thinking behavior
//
// These tests verify that thinking responses (like those from Claude with
// extended thinking) don't trigger premature task completion in one-mode.
//
// Key insight:
// - When a model produces ONLY thinking tokens (no visible text), this should
//   NOT immediately complete the task. Instead, we kick (call the model again)
//   up to 3 times to give it a chance to produce actual work output.
// - A thinking-only response is detected by: streaming_text is empty but
//   thinking_text is non-empty.
// - The completion logic is in decide_stream_end(has_text, mode, tool_count, kicks):
//   - has_text=false (only thinking) → kick up to 3 times before completing
//   - has_text=true (visible output) → different completion rules
//
// Buggy behavior (what we're testing against):
// - Thinking-only responses would cause immediate completion instead of a kick
// - This would waste the model's computation and leave tasks half-finished
// ============================================================================

#[cfg(test)]
mod thinking_e2e_tests {
    use super::*;

    // Test: Thinking-only response should NOT trigger completion (kick 1)
    #[test]
    fn thinking_only_response_first_kick() {
        // Scenario: Model outputs thinking token but no visible text.
        // has_text = false (no visible output)
        // This is the first empty kick
        let action = decide_stream_end(
            false,                  // has_text: only thinking, no visible output
            AppMode::One,           // one-mode
            0,                      // tool_iteration_count: no tools used yet
            0,                      // consecutive_empty_kicks: this is kick #1
        );
        
        // Should kick (continue) to give model a chance to produce output
        assert_eq!(action, StreamEndAction::Kick,
            "Thinking-only response should kick, not complete (kick #1)");
    }

    #[test]
    fn thinking_only_response_second_kick() {
        // Scenario: Model outputs ONLY thinking again on the second kick.
        // We still shouldn't complete yet.
        let action = decide_stream_end(
            false,                  // has_text: only thinking, no visible output
            AppMode::One,
            0,
            1,                      // consecutive_empty_kicks: this is kick #2
        );
        
        // Should still kick — give model more chances
        assert_eq!(action, StreamEndAction::Kick,
            "Thinking-only response should kick (kick #2)");
    }

    #[test]
    fn thinking_only_response_third_kick() {
        // Scenario: Model outputs ONLY thinking for the third time.
        // After 3 kicks with no visible output, we give up.
        let action = decide_stream_end(
            false,                  // has_text: only thinking, no visible output
            AppMode::One,
            0,
            2,                      // consecutive_empty_kicks: this is kick #3
        );
        
        // Now we complete — model has had 3 chances
        assert_eq!(action, StreamEndAction::CompleteOne("empty responses"),
            "After 3 thinking-only kicks, complete with 'empty responses'");
    }

    // Test: Visible text (even after thinking) should follow normal one-mode logic
    #[test]
    fn visible_text_after_thinking_no_tools_first_time() {
        // Scenario: Model outputs thinking + visible text (like "I'll write a function").
        // No tools have been used yet.
        // has_text = true (visible output exists)
        let action = decide_stream_end(
            true,                   // has_text: visible output after thinking
            AppMode::One,
            0,                      // tool_iteration_count: no tools used yet
            0,                      // consecutive_empty_kicks: this is first kick
        );
        
        // With visible text but no tools used yet, we should kick to nudge toward action
        assert_eq!(action, StreamEndAction::Kick,
            "Text-only response without tools should kick for action (1st kick)");
    }

    #[test]
    fn visible_text_after_thinking_no_tools_third_time() {
        // Scenario: Model keeps producing text but NO tool calls.
        // After 3 kicks with no action, we give up.
        let action = decide_stream_end(
            true,                   // has_text: visible output exists
            AppMode::One,
            0,                      // tool_iteration_count: no tools used yet
            2,                      // consecutive_empty_kicks: this is kick #3
        );
        
        // After 3 attempts with no tools, complete as "model unresponsive"
        assert_eq!(action, StreamEndAction::CompleteOne("model unresponsive"),
            "After 3 text-only kicks (no tools), complete as unresponsive");
    }

    #[test]
    fn visible_text_with_tools_completes_immediately() {
        // Scenario: Model outputs thinking + visible text + makes a tool call.
        // Tools were already called in a previous turn.
        let action = decide_stream_end(
            true,                   // has_text: visible output exists
            AppMode::One,
            3,                      // tool_iteration_count: tools were used before
            0,
        );
        
        // With tools already in play, visible text means task is done
        assert_eq!(action, StreamEndAction::CompleteOne("no tool calls"),
            "After tools were used, plain text response completes as done");
    }

    // Test: Plan/Ask modes always persist (never kick/complete)
    #[test]
    fn plan_mode_thinking_only_persists() {
        // In Plan mode, we never auto-kick or auto-complete.
        // Thinking-only response should just persist.
        let action = decide_stream_end(
            false,                  // only thinking
            AppMode::Plan,
            0,
        ...
        assert_eq!(action, StreamEndAction::Persist,
            "Plan mode with thinking-only should persist");
    }

    #[test]
    fn ask_mode_thinking_only_persists() {
        // In Ask mode (read-only), we never kick or complete.
        let action = decide_stream_end(
            false,                  // only thinking
            AppMode::Ask,
            0,
        ...
        assert_eq!(action, StreamEndAction::Persist,
            "Ask mode with thinking-only should persist");
    }

    // Test: Forever mode always kicks (never completes)
    #[test]
    fn forever_mode_thinking_only_keeps_going() {
        // Forever mode should never halt, even after many thinking-only responses
        let action = decide_stream_end(
            false,                  // only thinking
            AppMode::Forever,
            0,
            99,                     // even after many kicks
        );
        
        assert_eq!(action, StreamEndAction::Kick,
            "Forever mode with thinking-only should keep kicking");
    }

    #[test]
    fn forever_mode_with_visible_text_keeps_going() {
        let action = decide_stream_end(
            true,                   // visible output
            AppMode::Forever,
            5,                      // tools used
        ...
        assert_eq!(action, StreamEndAction::Kick,
            "Forever mode with visible text should still keep kicking");
    }

    // Test: Edge case - thinking interspersed with visible text
    #[test]
    fn mixed_thinking_and_visible_is_treated_as_visible() {
        // When thinking and visible text are both present, treat as visible=true
        // (because the model DID produce visible output to show to user)
        let action = decide_stream_end(
            true,                   // has_text: visible part exists despite thinking
            AppMode::One,
            0,
            0,
        );
        
        // Follow visible-text logic: kick to prompt for action
        assert_eq!(action, StreamEndAction::Kick,
            "Mixed thinking+visible response should follow visible-text logic");
    }

    // Test: Incomplete thinking block graceful handling
    #[test]
    fn incomplete_thinking_block_treated_as_thinking() {
        // Simulates: Parser received <think>processing... but no </think> before stream end.
        // Result: thinking_text has content, streaming_text is empty.
        // This is effectively thinking-only, so:
        let action = decide_stream_end(
            false,                  // has_text: incomplete think block leaves streaming_text empty
            AppMode::One,
            0,
            0,
        );
        
        // Should kick, just like any other thinking-only response
        assert_eq!(action, StreamEndAction::Kick,
            "Incomplete thinking block should kick (not crash)");
    }

    // Test: Full decision tree simulation (comprehensive one-mode scenario)
    #[test]
    fn one_mode_full_decision_tree() {
        // Kick 1: Model thinks deeply (no visible output)
        // Expected: Continue (kick)
        let mut action = decide_stream_end(false, AppMode::One, 0, 0);
        assert_eq!(action, StreamEndAction::Kick, "Kick 1: Thinking-only → continue");
        
        // Kick 2: Model still thinking (no visible output)
        // Expected: Continue (kick)
        action = decide_stream_end(false, AppMode::One, 0, 1);
        assert_eq!(action, StreamEndAction::Kick, "Kick 2: Thinking-only → continue");
        
        // Kick 3: Model outputs visible text ("I'll write a file"), but no tools yet
        // First time with text, so kicks_after=1, which is < 3
        // Expected: Continue (kick) - prompt model to actually make changes
        action = decide_stream_end(true, AppMode::One, 0, 0);
        assert_eq!(action, StreamEndAction::Kick, "Kick 3: Text-only (no tools) → continue");
        
        // Kick 4: Model finally calls a tool (e.g., "shell" command)
        // Now tool_iteration_count > 0
        // Model outputs visible text again
        // Expected: Complete - task done, model has taken action
        action = decide_stream_end(true, AppMode::One, 1, 0);
        assert_eq!(action, StreamEndAction::CompleteOne("no tool calls"), 
            "Kick 4: Text with tools → complete");
    }

    #[test]
    fn one_mode_decision_tree_unresponsive_model() {
        // Scenario: Model keeps outputting only thinking tokens, never produces visible output.
        // After 3 kicks, we should give up.
        
        let mut action;
        for i in 0..=2 {
            action = decide_stream_end(false, AppMode::One, 0, i as u32);
            if i < 2 {
                assert_eq!(action, StreamEndAction::Kick,
                    "Thinking-only kick {}: should continue", i + 1);
            } else {
                assert_eq!(action, StreamEndAction::CompleteOne("empty responses"),
                    "After 3 thinking-only kicks: should complete");
            }
        }
    }

    #[test]
    fn one_mode_decision_tree_unresponsive_with_text() {
        // Scenario: Model outputs text but never calls tools.
        // After 3 kicks with text but no tools, give up.
        
        let mut action;
        for i in 0..=2 {
            action = decide_stream_end(true, AppMode::One, 0, i as u32);
            if i < 2 {
                assert_eq!(action, StreamEndAction::Kick,
                    "Text-only (no tools) kick {}: should continue", i + 1);
            } else {
                assert_eq!(action, StreamEndAction::CompleteOne("model unresponsive"),
                    "After 3 text-only kicks (no tools): should complete");
            }
        }
    }
    ...
    #[test]
    fn consecutive_empty_kicks_wraps_safely() {
        // If somehow consecutive_empty_kicks reaches u32::MAX, don't panic or overflow
        let action = decide_stream_end(
            false,
            AppMode::One,
            0,
        ...
        // Should complete (kicks_after >= 3 is true)
        assert_eq!(action, StreamEndAction::CompleteOne("empty responses"),
            "Very high kick count should complete safely (no overflow)");
    }

    #[test]
    fn zero_tool_iteration_count_vs_positive() {
        // With tools: visible text → complete
        let action_with_tools = decide_stream_end(true, AppMode::One, 1, 0);
        assert_eq!(action_with_tools, StreamEndAction::CompleteOne("no tool calls"));
        
        // Without tools: visible text → kick
        let action_no_tools = decide_stream_end(true, AppMode::One, 0, 0);
        assert_eq!(action_no_tools, StreamEndAction::Kick);
    }

    // Test: Regression verification
    #[test]
    fn regression_thinking_only_no_premature_completion() {
        // The bug: thinking_text non-empty but streaming_text empty
        // would incorrectly complete the task immediately.
        // 
        // Fix: When has_text=false (no visible output),
        // we now kick up to 3 times before giving up.
        
        // First response: all thinking
        let action = decide_stream_end(
            false,                  // streaming_text is empty (only thinking)
            AppMode::One,
            0,
            0,
        );
        
        // BUG: Old code might have returned CompleteOne here
        // FIX: Must return Kick to continue
        assert_eq!(action, StreamEndAction::Kick,
            "REGRESSION: Thinking-only response MUST kick, never complete immediately");
    }

    #[test]
    fn regression_three_kicks_then_complete() {
        // The fix allows thinking-only responses to continue for up to 3 kicks.
        // After 3 kicks with no visible output, then complete.
        
        // Simulate: model produces only thinking for 3 turns
        let kick1 = decide_stream_end(false, AppMode::One, 0, 0);
        let kick2 = decide_stream_end(false, AppMode::One, 0, 1);
        let kick3 = decide_stream_end(false, AppMode::One, 0, 2);
        
        assert_eq!(kick1, StreamEndAction::Kick);
        assert_eq!(kick2, StreamEndAction::Kick);
        assert_eq!(kick3, StreamEndAction::CompleteOne("empty responses"),
            "REGRESSION: After 3 thinking-only kicks, must complete with exact reason");
    }
}
...
#[cfg(test)]
mod streamed_panels_tests {
    use crate::ui::panels::StreamedPanel;
    use tokio::sync::mpsc;
    ...
    #[test]
    fn test_streamed_panel_buffer_truncation() {
        let mut panel = StreamedPanel::new("test".to_string(), 0);
        // Add content up to near the limit
        let large_line = "x".repeat(1024 * 1024 - 100);
        panel.append_output(large_line);
        let size_before = panel.accumulated_output.len();
        assert!(size_before < 1024 * 1024);
        
        // Add more content that exceeds the limit
        let overflow_line = "y".repeat(200);
        panel.append_output(overflow_line);
        
        // Buffer should not exceed max
        assert!(panel.accumulated_output.len() <= 1024 * 1024);
        // Should still contain some of the overflow
        assert!(panel.accumulated_output.contains("yyy"));
    }
    ...
    fn test_streamed_panel_mark_finished() {
        ...
        panel.output_receiver = Some(mpsc::unbounded_channel().1);
    ...
    #[tokio::test]
    async fn test_streamed_panel_receiver_try_recv_single() {
        ...
        match rx.try_recv() {
            ...
            Err(_) => panic!("Expected message"),
        }
    }

    #[tokio::test]
    async fn test_streamed_panel_receiver_try_recv_multiple() {
        let (tx, mut rx) = mpsc::unbounded_channel();
        
        tx.send("Line 1".to_string()).unwrap();
        tx.send("Line 2".to_string()).unwrap();
        tx.send("Line 3".to_string()).unwrap();
    ...
    #[tokio::test]
    async fn test_streamed_panel_receiver_disconnect() {
        let (tx, mut rx) = mpsc::unbounded_channel();
        ...
        // Next try_recv should be empty
        let msg2 = rx.try_recv();
        assert!(matches!(msg2, Err(tokio::sync::mpsc::error::TryRecvError::Disconnected)));
    }

    #[test]
    fn test_streamed_panel_empty_output() {
        let mut panel = StreamedPanel::new("test".to_string(), 0);
        assert_eq!(panel.accumulated_output, "");
        assert!(!panel.is_streaming); // Default is false until explicitly set
        assert!(panel.exit_code.is_none());
    }
}

#[cfg(test)]
mod streamed_panel_cleanup_tests {
    use super::*;
    use crate::ui::panels::StreamedPanel;
    use tokio::process::Command;

    #[tokio::test]
    async fn test_kill_streamed_panel_with_running_process() {
        // Create a process that runs for 10 seconds (should be killed before timeout)
        ...
        // Create a streamed panel with the running process
        let mut panel = StreamedPanel::new("sleep_panel".to_string(), 0);
        panel.process = Some(child);
        panel.is_streaming = true;

        // Simulate removing the panel and killing it
        let mut panel_layout = crate::ui::panels::PanelLayout::new(100, 24);
        panel_layout.streamed_panels.insert("sleep_panel".to_string(), panel);

        // Kill the panel
        if let Some(removed_panel) = panel_layout.streamed_panels.remove("sleep_panel") {
            if let Some(mut child) = removed_panel.process {
                let _ = child.kill().await;
                let _ = child.wait().await;
            }
        }

        // Verify process is dead by checking if we can still get its status
        // (This is a best-effort check; the process should definitely be gone)
        std::thread::sleep(std::time::Duration::from_millis(100));
    }

    #[tokio::test]
    async fn test_kill_streamed_panel_already_finished() {
        ...
        let panel = StreamedPanel::new("echo_panel".to_string(), 0);

        // Kill should handle finished process gracefully
        let mut panel_layout = crate::ui::panels::PanelLayout::new(100, 24);
        
        let mut panel_mut = panel;
        panel_mut.process = Some(child);
        panel_mut.is_streaming = false; // Already finished
        
        panel_layout.streamed_panels.insert("echo_panel".to_string(), panel_mut);

        if let Some(removed_panel) = panel_layout.streamed_panels.remove("echo_panel") {
            if let Some(mut child) = removed_panel.process {
                // This should not panic even though process is already done
                let _ = child.kill().await;
            }
        }
    }
    ...
    #[test]
    fn test_remove_streamed_panel_decrements_count() {
        let mut panel_layout = crate::ui::panels::PanelLayout::new(100, 24);

        // Create two streamed panels
        let panel1 = StreamedPanel::new("panel1".to_string(), 0);
        let panel2 = StreamedPanel::new("panel2".to_string(), 0);

        panel_layout.streamed_panels.insert("panel1".to_string(), panel1);
        panel_layout.streamed_panels.insert("panel2".to_string(), panel2);
        panel_layout.active_stream_count = 2;

        // Remove one panel
        let removed = panel_layout.remove_streamed_panel("panel1");
        assert!(removed.is_some());
        assert_eq!(panel_layout.active_stream_count, 1);
        assert_eq!(panel_layout.streamed_panels.len(), 1);

        // Remove the other panel
        let removed = panel_layout.remove_streamed_panel("panel2");
        assert!(removed.is_some());
        assert_eq!(panel_layout.active_stream_count, 0);
        assert_eq!(panel_layout.streamed_panels.len(), 0);
    }
    ...
    fn test_remove_nonexistent_streamed_panel() {
        ...
        // Try to remove a panel that doesn't exist
        let removed = panel_layout.remove_streamed_panel("nonexistent");
        assert!(removed.is_none());
    }

    #[tokio::test]
    async fn test_multiple_streamed_panels_cleanup() {
        // Create multiple quick-finishing processes
        let mut panel_layout = crate::ui::panels::PanelLayout::new(100, 24);
        ...
        for i in 0..3 {
            ...
            let panel_name = format!("panel{}", i);
            let mut panel = StreamedPanel::new(panel_name.clone(), 0);
            panel.process = Some(child);

            panel_layout.streamed_panels.insert(panel_name, panel);
        }

        assert_eq!(panel_layout.streamed_panels.len(), 3);

        // Clean up all panels
        let panel_names: Vec<String> = panel_layout.streamed_panels.keys().cloned().collect();
        for panel_name in panel_names {
            if let Some(panel) = panel_layout.streamed_panels.remove(&panel_name) {
                if let Some(mut child) = panel.process {
                    let _ = child.kill().await;
                    let _ = child.wait().await;
                }
    ...
    #[tokio::test]
    async fn test_process_cleanup_timeout() {
        // Create a process that would be killed if not terminated by SIGTERM
        let child = Command::new("sleep")
            .arg("30")
            .stdout(std::process::Stdio::piped())
            .spawn()
        ...
        let mut panel = StreamedPanel::new("timeout_test".to_string(), 0);
        panel.process = Some(child);

        // Simulate the kill with timeout logic
        ...
        if let Some(mut child) = panel.process {
            ...
            // Process should be dead either from SIGTERM or timeout
            match timeout {
                Ok(_) => {
                    // Terminated within timeout
                    let elapsed = start.elapsed();
                    assert!(elapsed < std::time::Duration::from_secs(3));
                }
                Err(_) => {
                    // Timeout occurred, but SIGKILL should still take effect
                }
            }
        }
    }
